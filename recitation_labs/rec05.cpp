...
/*
Code Question:
Compilation error at line C, this is because mem class requires an integer 
to initialize the class, but in line C, we just have a member with no initialization
int data. We can do : member(rhs.member) instead since it initializes rhs.member as our
integer data
