...
//  They are implemented below the main function. 
//    
...
Node* reverse(const Node* hp) {
    Node* end = nullptr; // start of a new list
    const Node* current = hp; // current = first node of llist; cannot change value

    while (current != nullptr) { // traversing through llist
        Node* newNode = new Node{current->data, end};
        end = newNode; // sets the newNode as the new end for next iteration
        current = current->next; 
    }
    return end; 
}

void reverseInPlace(Node*& hp) { // modifies in place since using by ref "&"
    Node* prev = nullptr; // start of a new list 
    Node* current = hp; // can change value this time 

    while (current != nullptr) { // traversing through llist 
        Node* next = current->next; // 'next' is the current node's next node
        current->next = prev; // swapping from (left (prev)) of current to the back 
        prev = current; 
        current = next; 
    } 
    hp = prev; 
...
const Node* isSublist(const Node* pattern, const Node* target) {
    if (target == nullptr) { // if sublist is empty (nullptr)
        return pattern; // returns the first list 
    }

    const Node* currentTarget = target; // setting target as FIRST target node

    while (currentTarget != nullptr) { // traversing through llist (target)
        // setting current nodes for pattern/target; can ONlY change the address
        const Node* patternPtr = pattern;
        const Node* targetPtr = currentTarget; 
        bool match = true; // assume there is 

        // // while not done traversing
        while (patternPtr != nullptr && targetPtr != nullptr) { 
            if (patternPtr->data != targetPtr->data) {
                match = false; // not a match, does not traverse through patternPtr
                break; 
            }
            // traversal
        ...
        if (match && patternPtr == nullptr) { // if traversal ended & match: true
            return currentTarget; // return first node of pattern
        }

        currentTarget = currentTarget->next; // if current node != start of sublist
    }
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB) {
    const Node* currentA = listA; 

    while (currentA) { 
    // not listA still traversable (checking one node at a time for A / B)
        const Node* currentB = listB; 
        while (currentB != nullptr) { // make sure listB still traversable
            if (currentA == currentB) { // compare addresses 
                return currentA;
            }
            currentB = currentB->next; // traversal for B
        }
        currentA = currentA->next; // traversal for A
    }
    return nullptr; 
}
...
const Node* sharedListUsingSet(const Node* listA, const Node* listB) {
    unordered_set<const Node*> setOfNodes; // makes sure pointers dont change

    // add nodes from listA into set:
    const Node* current = listA; // current items for listA
    while (current != nullptr) {
        setOfNodes.insert(current); 
        current = current->next; // traversal
    }

    // check if listb items are on the set
    current = listB; // now current items for listB
    while (current != nullptr) {
        if (setOfNodes.find(current) != setOfNodes.end()) { // if we CAN find item in set
            return current;
        }
        current = current->next; // traversal
    }
